        ...
        <div className="font-poppins flex flex-row gap-10 bg-[#E4E4E4] w-[750px] h-[600px] rounded-xl object-center align-center justify-center font-poppins">
            <div>
                ...
                <div className="overflow-y-scroll w-[200px] h-[80%] mt-[3vw]">
                    {tops.map((item, index) => (
                        <div key={index} onClick={() => setSelectedTopIndex(index)} className={`item mb-2 w-[150px] rounded-lg h-[150px] bg-white text-white ${selectedTopIndex === index ? 'bg-blue-500' : ''}`}>{item}</div>
                    ))}
                </div>
            </div>
            <div>
                ...
                <div className="overflow-y-scroll w-[200px] h-[80%] mt-[3vw]">
                    {bottoms.map((item, index) => (
                        <div key={index} onClick={() => setSelectedBottomIndex(index)} className={`item mb-2 w-[150px] rounded-lg h-[150px] bg-white text-white ${selectedBottomIndex === index ? 'bg-blue-500' : ''}`}>{item}</div>
                    ))}
                </div>
            </div>
            <div>
                ...
                <div className="overflow-y-scroll h-[80%] w-[200px] mt-[3vw]">
                    {accessories.map((item, index) => (
                        <div key={index} onClick={() => setSelectedAccessoryIndex(index)} className={`item mb-2 w-[150px] rounded-lg h-[150px] bg-white text-white ${selectedAccessoryIndex === index ? 'bg-blue-500' : ''}`}>{item}</div>
                    ))}
                </div>
            </div>
        </div>
